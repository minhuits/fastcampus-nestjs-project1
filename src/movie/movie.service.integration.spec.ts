...
describe('MovieService - Integration Test', () => {
  ...
  beforeEach(async () => {
    // await cacheManager.reset(); // 이유를 모르는 ERROR가 발생함
  ...
  describe('create', () => {
    beforeEach(() => {
      jest.spyOn(service, 'createRenameMovieFile').mockResolvedValue();
    });
    ...
    it('should create movie correctly', async () => {
      ...
      const result = await service.create(createMovieDto, users[0].id, dataSource.createQueryRunner()) as Movie;
  ...
  describe('update', () => {
    it('should update movie correctly', async () => {
      ...
      const result = await service.update(movieId, updateMovieDto) as Movie;
